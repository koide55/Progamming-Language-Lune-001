...
`=` は代入または束縛構文で使う。等価比較は `==`。`??` は null 合体（左辺は nullable）、`?.` はセーフナビゲーション。詳細は `LANGUAGE_SPEC.md`。
...
- 結果の型は `x op e` と同じ規則で決まる。`/` は常に真の除算なので `/=` の結果は `Double` であり、`Int` の変数への `x /= 2` は型エラーになる。
- `x /= 0` や `x %= 0` は `x / 0` / `x % 0` と同じく `RUN0006`（ゼロ除算）になる。
- `+` は String 同士も受け付けるため、`s += "..."` は文字列連結になる。
- 切り捨て除算演算子 `//` は v0.1 に存在しないので、`//=` も存在しない。
