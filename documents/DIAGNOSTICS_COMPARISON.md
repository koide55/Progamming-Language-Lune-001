...
Lune 側の入力は各ケースの「Lune」節に全文を掲載してある。任意の `.lune`
ファイルに保存して次を実行すれば再現できる:

```
./bin/lune --check --lang ja <file>.lune     # ケース1-3(型検査で報告)
./bin/lune --eval a --lang ja <file>.lune    # ケース4(強制時に報告)
```
