...
なお、本書が掲げる目標機能のうち、以下は既に v0.1 で実装済みである（詳細は `LANGUAGE_SPEC.md`）: デフォルト遅延評価、代数的データ型・パターンマッチ・match 網羅性チェック、`T?`（null 非許容 / nullable と null 安全）、パイプライン `|>`、ローカル型推論、正準フォーマッタ `lune fmt`。本書はこれらを含む長期目標を一貫した形で記述するため、記述が「将来」の体裁でも既実装のものがある点に留意すること。
