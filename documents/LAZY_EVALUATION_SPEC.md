...
`==` / `!=` は構造的等価で判定する（`LANGUAGE_SPEC.md` §9.1.1）。比較は値を観測する操作なので、表示と同じく必要な部分を force する。

- 両辺を左から順に比較し、比較対象になった部分だけを force する。
- 最初の不一致が見つかった時点で打ち切り、残りの要素・フィールドは force しない。

```lune
[1, crash()] == [2, crash()]   -- false（先頭で不一致。crash() は評価されない）
take(repeat(7), 3) == [7, 7, 7]  -- true
```

不一致が見つからない限り両辺全体（`deepForce` 相当）を force するので、無限リスト同士の比較は停止しない。これは仕様である。無限構造を比較したいときは `take` などで有限の部分を切り出してから比較する。
...
- `take` の返す tail の遅延。
- `==` / `!=` の構造的等価（必要な分だけ force、最初の不一致で打ち切り）。
