...
複合代入演算子 `+= -= *= /= %=` が使える。`x op= e` は `x = x op e` と同じで、型も実行時の意味も `x op e` に一致する。したがって `x /= 2` の結果は `Double` であり (`/` は常に真の除算)、`Int` の変数に対しては型エラーになる。ゼロ除算は `x / 0` / `x % 0` と同じ `RUN0006` を報告する。詳細は `SYNTAX_SPEC.md` 14.1 節。
