...
> **v0.1 の注意** — ADT のコンストラクタは**位置引数**で呼びます（`Income("salary", 3000)`）。`Income(label = "salary", amount = 3000)` と名前付きで書くこともできてしまいますが、**名前は照合されず位置で解釈される**ため、順序を入れ替えると値が入れ違います。危険なので、ADT では名前付きを使わないでください（名前付きが必須なのはレコード、第6章）。

## 13.3 数列の実験室 — 無限リストと評価回数の観察

最後は遅延評価の総合演習です。コラッツ数列（偶数なら半分、奇数なら3倍して1を足す。1に到達するまで続く）を無限リストで作ります。

ここで v0.1 の壁に当たります。**整数除算がありません** — `/` は常に `Double` を返すので（第2章）、`n / 2` は `Int` になりません。`%` はあるのに商が取れないのです。

道具がなければ作ります。第9章の `var` と `while` の実戦です。`collatz.lune`:

```lune
module collatz

# v0.1 の `/` は常に Double を返し、整数除算がない。
# 必要な道具は自分で作る — 第9章の var と while の実戦。
def halve(n: Int): Int =
    var rest = n
    var count = 0
    while rest >= 2:
...
def next(n: Int): Int =
    if n % 2 == 0 then halve(n) else 3 * n + 1
...
5. **道具がなければ作る**（`halve` のように。言語が小さいのは弱点ではない）
...
**演習 13-4**（★★★） `halve` は2ずつ引くので、大きな数では遅くなります。`while` を使って、もっと速い整数除算 `divide(a: Int, b: Int): Int` を書いてください（`b > 0` を仮定してよい）。

<details><summary>解答</summary>

素直な版は「`b` を引ける回数を数える」形で、`halve` の一般化です。
...
これでも `a / b` に比例した時間がかかります。速くするなら「`b` を2倍しながら大きく削る」（倍加法）ですが、v0.1 の道具立てでは実装が煩雑になります。ここで気づいてほしいのは、**これは言語が埋めるべき穴**だということ。整数除算は基本的な演算で、ライブラリではなく言語側にあるべきです（付録E で触れる将来課題の一つです）。
