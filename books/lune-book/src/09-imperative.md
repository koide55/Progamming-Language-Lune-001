...
代入できるのは名前だけです。`i = i + 1` と書くほかに、複合代入 `+= -= *= /= %=` も使えます（`i += 1` は `i = i + 1` とまったく同じ意味です）。ひとつ落とし穴があって、`/` は常に真の除算なので `Int` の変数への `x /= 2` は型エラーになります（`Double` になってしまうため）。整数を整数のまま割りたいときは、床除算を使って `x = x // 2` と書きます（`//` は 13.3 節）。
