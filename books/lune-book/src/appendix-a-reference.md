...
ブロックはインデントで表します。行頭に使えるのは**スペースだけ**です。

> **タブの扱い（v0.1 の実装の話）** — 診断コード `LXL0004`「インデントにタブは使えない」
> は用意されていますが、**v0.1 の実装では出ません**。インデント幅を数えるときに先頭の
> スペースだけを見るので、タブはインデントとして数えられず、その行は「インデント 0」と
> 見なされます。結果は場合により、`PRS0002`（INDENT が必要）になることもあれば、
> そのまま通ってしまうこともあります。**タブは使わないでください** —
> 意図した診断が出ないので、原因の分かりにくいエラーになります。
...
書きながら仕様書と実装が食い違っている箇所を2つ見つけたので、この付録は**実装の側**に
合わせてあります。`LANGUAGE_SPEC.md` §5 は文字リテラルの型を `Char` としていますが実際は
`String` であり、同 §4 は「タブによるインデントはエラー」としていますが `LXL0004` は
実装上出ません。どちらを直すか（仕様に寄せるか実装に寄せるか）は言語側の判断です。
