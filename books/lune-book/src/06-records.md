...
ここは ADT のコンストラクタ（位置渡し、第5章）と対照的です。レコードはフィールドが増えがちで、`User("Ada", 36)` の 36 が何かは型が同じ限り機械にも人にも判別できない — だから名前を強制する、という設計です。おかげで構築まわりの間違いは、すべて名指しで叱ってもらえます。足りなければ `REC0003`（フィールドが不足）、知らない名前なら `REC0005`（宣言されていないフィールド）、同じ名前を2回書けば `REC0004`（フィールドの二重指定）です。
