...
「保存」ボタン(または `Ctrl/Cmd+S`)で、エディタの内容を `.lune`
ファイルとしてローカルに書き出す。ブラウザ内で完結し、サーバーには何も送らない。

- **File System Access API** が使えるブラウザ(Chromium 系)ではネイティブの保存
  ダイアログが開き、保存先とファイル名をその場で選べる。
- 使えないブラウザ(Safari / Firefox)では `main.lune` という名前のダウンロードに
  フォールバックする。API 名だけあって権限が下りない場合も同じ経路に落ちる。
- 保存する内容には**末尾改行を補う**。付けないと、書き出したファイルが
  `lune fmt --check` で `would reformat` になる(ユーザーが打っていない理由で
  CI が落ちてしまう)。

`main.lune` を既定名にしているのは、診断が `main.lune:行:列` と表示するのと
揃えるため。
...
5. 保存の逆 — ローカルの `.lune` をエディタに読み込む (今は保存のみ)
